...
- 分析前计算设计图与开发图的宽高比差异。
- 差异不超过 `1%` 时，在内存中将开发图归一化到设计图尺寸，不覆盖原始素材。
- 差异超过 `1%` 时，在创建验收记录前严格拦截，并返回两张图尺寸、差异比例和目标尺寸。
- 工作台使用 Ant Design Vue 通知展示可理解的中文错误；自动对齐成功后持续展示原尺寸、目标尺寸和缩放比例。
- OCR 使用临时归一化图片，分析完成后立即清理，避免文字框坐标基于错误尺寸。
- 正常启动内置 Python 时禁用字节码写入，避免运行后的公开安装目录出现构建机路径缓存。

对抗式审查发现并修复：

1. `1%` 边界受浮点误差影响而被误判为超限。
2. 尺寸响应误复用完整素材对象，存在本地路径外泄风险；现仅返回 `width/height`。
3. 超阈值预检必须发生在创建 run 之前，避免失败历史污染。
4. OCR 不能继续读取未归一化原图，否则文字差异坐标会与像素分析不一致。
5. 应用启动后不应在可分发目录生成 `.pyc/__pycache__`。
...
| --- | --- |
| TypeScript 类型检查 | 通过 |
| TypeScript 测试 | 18 个测试文件，42/42 通过 |
| Python 视觉引擎测试 | 11/11 通过 |
| Chrome 扩展构建 | 通过 |
| Vue 工作台构建 | 通过 |
| Node 本地服务构建 | 通过 |
...
| 平均坐标误差 | 0.0 px |
| 可搬迁运行 | 通过 |
| 包级路径与软链接检查 | 15,644 个文件；0 个绝对软链接；0 个私有路径泄漏 |
...
- 产品品牌统一为 `Vigour UI Review`，MIT 版权署名现为 `LukiDesign`（2026-09-16 按用户要求更新，见第 13 节）。
...
## 13. 发布商与版权署名统一（2026-09-16）

- 按用户明确要求，发布商名称与本工程版权署名统一为 `LukiDesign`。
- 同步 LICENSE、中英文 README、项目维护者说明、中英文隐私政策、商店材料和材料生成脚本。
- 保留 MIT 许可条款、第三方版权声明、产品名 `Vigour UI Review`、仓库地址、扩展标识和版本号。
- 此次只更新源码与新生成的材料包；不覆盖历史 Release、旧材料包、现有安装，也未修改 Chrome 商店后台。
