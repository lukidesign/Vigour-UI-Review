...
当前源码版本 0.0.1，仅用于首次创建商店草稿、取得该条目的固定扩展 ID。它不是 GitHub 旧版安装包，也没有覆盖已发布的 GitHub Release。扩展依赖配对的 Native 配套程序，不能独立完成本地分析。

真实工具栏采集、新机器安装、后台/休眠和签名等验收未完成，**不要点击提交审核或公开发布**。下一次上传商店包需要更高版本号，届时须同步扩展和配套程序并重新测试。
