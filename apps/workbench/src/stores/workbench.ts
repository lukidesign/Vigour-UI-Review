...
import type { AnalysisResponse, ImageAsset, Issue, Project, Run } from '../types';
...
export const useWorkbenchStore = defineStore('workbench', () => {
  ...
  async function selectProject(id: string) {
    ...
    else { activeRun.value = undefined; issues.value = []; evidenceAssetId.value = ''; }
  }
  ...
  async function importFigma(url: string) {
    ...
    designAsset.value = result.asset; return result;
  }
  ...
  return { projects, activeProjectId, activeProject, runs, activeRun, issues, evidenceAssetId, designAsset, implementationAsset, captures, busy, error, loadProjects, refreshCaptures, useCapture, createProject, selectProject, selectRun, uploadImage, analyze, updateIssue, figmaStatus, saveFigmaPat, importFigma, aiKeyStatus, saveAIKey, analyzeWithAI };
});
